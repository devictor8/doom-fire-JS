...
const calculateFirePropagation = () => {
    for (let column = 0; column < fireWidth; column++) {
        for (let row = 0; row < fireHeight; row++) {
            pixelIndex = column + (row * fireWidth)
...
const updateFireIntesity = (currentPixelIndex) => {
    ...
    firePixelsArray[eval(`currentPixelIndex ${windSide} decay`)] = newFireIntesity
}
...
const createFireSource = () => {
    for (let column = 0; column <= fireWidth; column++) {
        ...
        firePixelsArray[pixelIndex] = fireIntesityNumber
    }
}

document.querySelector('#lessButton').addEventListener('click', ()=>{
    if (fireIntesityNumber > 0) {
        fireIntesityNumber -= 6
        createFireSource()
    }
})

document.querySelector('#plusButton').addEventListener('click', ()=>{
    if (fireIntesityNumber < 36) {
        fireIntesityNumber += 6
        createFireSource()
    }
})
document.querySelector('#maxButton').addEventListener('click', ()=>{
    fireIntesityNumber = 36
    createFireSource()
})

document.querySelector('#minButton').addEventListener('click', ()=>{
    fireIntesityNumber = 0
    createFireSource()
})

document.querySelector('#leftWind').addEventListener('click', ()=>{
    windSide = '-'
    createFireSource()
})

document.querySelector('#rightWind').addEventListener('click', ()=>{
    windSide = '+'
    createFireSource()
})
